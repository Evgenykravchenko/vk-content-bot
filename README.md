...
Подробная пошаговая инструкция для редактора: **[Как управлять контентом бота](docs/CONTENT_GUIDE.md)**.
...
- настраиваемый fallback для неизвестных сообщений: текст или полноценный ответ;
...
- автоматическое создание чистой структуры Directus без демонстрационного контента;
...
При первом запуске контейнер `cms-bootstrap` создаст шесть пустых пользовательских разделов. Его состояние `Exited (0)` после инициализации — нормальное поведение. Демонстрационный контент не создаётся, если явно не установить `DIRECTUS_SEED_DEMO=true`.

## Как добавить первый ответ

Полная редакторская инструкция со всеми вариантами — в файле **[docs/CONTENT_GUIDE.md](docs/CONTENT_GUIDE.md)**. Ниже оставлена короткая памятка.
...
| `DIRECTUS_SEED_DEMO`          | Создавать ли тестовую команду `помощь`           | нет, `false` |
